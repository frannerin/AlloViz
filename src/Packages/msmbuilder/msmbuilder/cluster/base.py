...
class MultiSequenceClusterMixin(object):
    ...
    def fit(self, sequences, y=None, sample_weight=None): # Changed by frannerin
        ...
        super(MultiSequenceClusterMixin, self).fit(self._concat(sequences), sample_weight=sample_weight) # Changed by frannerin
    ...
    def fit_predict(self, sequences, y=None, sample_weight=None): # Changed by frannerin
        ...
        if hasattr(super(MultiSequenceClusterMixin, self), 'fit_predict'):
            check_iter_of_sequences(sequences, allow_trajectory=self._allow_trajectory)
            labels = super(MultiSequenceClusterMixin, self).fit_predict(sequences, sample_weight=sample_weight) # Changed by frannerin
        else:
            self.fit(sequences)
            labels = self.predict(sequences, sample_weight=sample_weight) # Changed by frannerin
